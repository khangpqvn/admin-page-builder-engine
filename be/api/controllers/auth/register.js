    ...
    try {
      ...
      if (exitsAccount) {
        return res.forbidden({
          message: sails.__('The logon name already exists, please use another username!')
        })
      }

      let defaultUserType = Conf.get('DEFAULT_USERTYPE_ON_REGISTING') || 3;
      let roleId = []
      let emailDomain = inputs.email.toLowerCase().split('@')[1]
      if ((((Conf.get('FPT_EDU_MAIL_DOMAIN_LIST') || '') + "").toLowerCase().split(',')).includes(emailDomain) && Conf.get("ADDITIONAL_FPT_EDU_ROLE")) {
        roleId.push(Conf.get("ADDITIONAL_FPT_EDU_ROLE"))
      }
      let {
        auth,
        user
      ...
      await sails.helpers.cache.set('authActivate_' + token, JSON.stringify(authActivate), 24 * 60 * 60);

      let link = Conf.get('ACTIVE_ACCOUNT_URL').replace('{{token}}', token).replace('{{account}}', auth.key);
...
------------------------------------------- 
        `
      pushNotification.sendMail(user.email, sails.__('[Thông báo] - Kích hoạt tài khoản OBT!'), content, req, {}).catch(e => {
        log.error(e, req)
      });
      return exits.success({
        message: sails.__('Vui lòng kiểm tra hòm mail để kích hoạt tài khoản!')
      });
    } catch (e) {
