...
nrp.on(sails.config.datastores.redis.prefix + '_refresh_cache', function (data) {

  if (data && data.modelName && sails.models[data.modelName] && sails.models[data.modelName].refreshCache) {
    sails.models[data.modelName].refreshCache().then().catch(err => {
      log.error(err);
    });
  }
});
