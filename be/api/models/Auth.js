...
module.exports = {
  ...
  beforeCreate: function (createData, cb) {
    ...
    if (createData.password) {
      if (!sails.helpers.common.checkPasswordStrength(createData.password)) {
        return cb((sails.__('Mật khẩu phải bao gồm cả chữ và số đồng thời ít nhất 6 ký tự!')));
      }
      createData.password = sails.helpers.common.hash(createData.password);
    } else {
  ...
  beforeUpdate: function (updateData, cb) {
    ...
    if (updateData.password === '') {
      return cb(sails.__('Mật khẩu không được để trống'));
    }
    if (updateData.password) {
      if (!sails.helpers.common.checkPasswordStrength(updateData.password)) {
        return cb(sails.__('Mật khẩu phải bao gồm cả chữ và số đồng thời ít nhất 6 ký tự!'));
      }
      updateData.lastChangePasswordAt = moment().valueOf();
      updateData.password = sails.helpers.common.hash(updateData.password);
    } else {
...
function validatePassword(auth, password) {
  return sails.helpers.common.checkHash(password, auth.password);
}
...
async function login(account, password, type, authId) {
  ...
  if (authId) { //using for generate token from admin page or refresh token
    where.id = authId;
  } else {
...
async function changePassword(account, oldPassword, newPassword, type) {
  try {
    ...
    if (!sails.helpers.common.checkPasswordStrength(newPassword)) {
      return {
        status: false,
        message: sails.__('The new password must include both letters and numbers and at least 6 characters long!')
      };
    }
    ...
    return {
      status: true,
      message: sails.__('Update password success!')
    };
  } catch (e) {
    log.error(e);
    return {
...
async function resetPassword(account, type) {
  try {
    let password = Conf.get('DEFAULT_PASSWORD') || 'abc1234';
    await Auth.update({
      key: account,
      type
    }).set({
