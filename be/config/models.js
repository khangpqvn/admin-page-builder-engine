...
module.exports.models = {
  ...
  afterCreate: async function (v, cb) {
    let modelName = v.toJSON().modelName;
    if (modelName && sails.models[modelName].setCache) {
      // sails.models[v.toJSON().modelName].refreshCache().then().catch(err => {
      //   log.error(err);
      // });
      try {
        await sails.models[modelName].setCache();
  ...
  refreshCache: async function () {
    ...
    console.log('Refresh cache ' + modelName);
    try {
      if (!await sails.helpers.cache.get('cache_' + modelName) ) throw new Error("Missing caching")
      this.cacheData = JSON.parse(await sails.helpers.cache.get('cache_' + modelName));
    } catch (error) {
  ...
  setCache: async function () {
    ...
    sails.helpers.cache.set('cache_' + modelName, JSON.stringify(dataCache));
    console.log('Done set cache ' + modelName);
    let nrp = sails.helpers.pubsub.nrp();
  ...
  removeUneccessaryValue: function (obj) {
    ...
    let omits = ['isDelete', 'createdBy', 'createdAt', 'updatedAt', 'deletedAt', 'deletedBy'];
